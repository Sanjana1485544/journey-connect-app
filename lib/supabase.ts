...
// Some users accidentally paste a trailing '.' after the project ref (e.g. "...supabase.co.")
// which breaks DNS. Strip trailing dots and slashes.
const CLEAN_SUPABASE_URL = SUPABASE_URL?.trim().replace(/[/.]+$/, '');
const CLEAN_SUPABASE_ANON_KEY = SUPABASE_ANON_KEY?.trim();

if (!CLEAN_SUPABASE_URL || !CLEAN_SUPABASE_ANON_KEY) {
  throw new Error('Missing Supabase environment variables');
}

const getNetworkError = (error: unknown, fallback: string) => {
  const message = error instanceof Error ? error.message : '';
  if (message.includes('Network request failed') || message.includes('ENOTFOUND')) {
    return new Error('Unable to reach backend. Please verify SUPABASE_URL and internet connectivity.');
  }
...
export const getHeaders = async () => {
  ...
  const headers: Record<string, string> = {
    ...
    'apikey': CLEAN_SUPABASE_ANON_KEY,
  ...
  if (session) {
    try {
      ...
      if (
        ...
        sessionData.access_token !== CLEAN_SUPABASE_ANON_KEY
      ) {
...
export const auth = {
  signIn: async (email: string, password: string) => {
    try {
      const response = await fetch(`${CLEAN_SUPABASE_URL}/auth/v1/token?grant_type=password`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'apikey': CLEAN_SUPABASE_ANON_KEY,
      ...
      if (!response.ok) {
        const apiMessage = getApiErrorMessage(data, `Sign in failed (${response.status})`);
      ...
      const normalizedError = getNetworkError(error, 'Sign in failed');
      console.error('Error signing in:', normalizedError);
  ...
  signUp: async (email: string, password: string) => {
    try {
      ...
      const response = await fetch(`${CLEAN_SUPABASE_URL}/auth/v1/signup`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'apikey': CLEAN_SUPABASE_ANON_KEY,
      ...
      if (!response.ok) {
        const apiMessage = getApiErrorMessage(data, `Sign up failed (${response.status})`);
        throw new Error(apiMessage);
      }
      ...
      const normalizedError = getNetworkError(error, 'Sign up failed');
      console.error('Error signing up:', normalizedError);
      return { data: null, error: normalizedError };
    }
  },
  ...
  signOut: async () => {
    try {
      ...
      await fetch(`${CLEAN_SUPABASE_URL}/auth/v1/logout`, {
        method: 'POST',
        headers,
      });
  ...
  getSession: async () => {
    try {
      ...
      const userResponse = await fetch(`${CLEAN_SUPABASE_URL}/auth/v1/user`, {
        headers: {
      ...
      const profileResponse = await fetch(
        `${CLEAN_SUPABASE_URL}/rest/v1/users?id=eq.${userData.id}&select=*`,
        { headers }
      );
...
export const supabase = {
  from: (table: string) => ({
      ...
      const queryBuilder = {
        ...
        execute: async () => {
          try {
            ...
            const url = `${CLEAN_SUPABASE_URL}/rest/v1/${table}?${queryParams.toString()}${filterString ? `&${filterString}` : ''}${limit}`;
    ...
    insert: async (data: any) => {
      try {
        const headers = await getHeaders();
        const url = `${CLEAN_SUPABASE_URL}/rest/v1/${table}`;
          ...
          return {
            execute: async () => {
              try {
                ...
                const url = `${CLEAN_SUPABASE_URL}/rest/v1/${table}?${filterString}`;
                const response = await fetch(url, {
                  method: 'PATCH',
                  headers,
                  body: JSON.stringify(data),
